...
3. Idle detection uses byte activity timestamps, not process liveness only.
...
1. `transcript.bin` or `transcript.txt` (framed events)
2. `solution.stderr.log`
3. `interactor.stderr.log`
4. `runner.events.jsonl` (timestamped state transitions)
...
1. RFC-003: Sandbox and isolation model
2. RFC-004: Grader/two-step ABI contract and multi-language support
3. RFC-005: Scoring model and subtask aggregation semantics
